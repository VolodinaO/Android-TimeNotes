...
public class CameraActivity extends Activity implements SurfaceHolder.Callback, View.OnClickListener, Camera.PictureCallback, Camera.PreviewCallback, Camera.AutoFocusCallback
{
    ...
    public void onPictureTaken(byte[] paramArrayOfByte, Camera paramCamera)
    {
        ...
        try
        {
            ...
            Intent in = new Intent(CameraActivity.this,RecordActivity.class);
            String s = intent.getStringExtra(RecordActivity.STATE);
            if(s.length()==3)
                in.putExtra(RecordActivity.STATE,"new");
            else in.putExtra(RecordActivity.STATE,"open");
            in.putExtra(RecordActivity.PHOTO,path);
            in.putExtra(RecordActivity.RECORD,intent.getSerializableExtra(RecordActivity.RECORD));
            startActivity(in);
        }
        catch (Exception e)
        {
